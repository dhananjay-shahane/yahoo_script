...
# Use the cloud database URL instead
...
def create_table_if_not_exists(symbol, table_type):
    ...
    table_name = f"{symbol}{CONFIG[table_type]['table_suffix']}"
    ...
    try:
        ...
        cur.execute("CREATE SCHEMA IF NOT EXISTS symbols;")
        conn.commit()
        ...
        if not exists:
            ...
            cur.execute(f"""
                ...
                CREATE INDEX idx_{table_name.lower()}_datetime ON symbols.{table_name} (datetime);
            """)
            conn.commit()
...
def save_to_database(symbol, df, table_type):
    ...
    try:
        ...
        if new_data:
            ...
            """, new_data)
            conn.commit()
            print(f"Inserted {cur.rowcount} new records to symbols.{table_name}")
