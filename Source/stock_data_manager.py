...
class StockDataManager:
    ...
    def add_multiple_symbols(self, symbols):
        ...
        for i, symbol in enumerate(symbols, 1):
            print(f"\n[{i}/{total_symbols}] Processing symbol: {symbol}")
            print("-" * 40)

            # Add delay between symbols to avoid rate limiting (except for first symbol)
            if i > 1:
                delay = 5  # 5 seconds between symbols
                print(f"⏳ Waiting {delay} seconds before processing next symbol...")
                time.sleep(delay)
            ...
            print(f"📊 Progress: {i}/{total_symbols} ({progress:.1f}%)")
